...
export default function AnalyticsPage() {
  const { model, issuesData, govLoading, advanceAnalyticsLoading, advanceAnalyticsComplete, runFullAnalytics, pullsData, auditComplete, loading, selectedOrg, analyticsError, rateLimit } = useApp()
  ...
  const scopedIssueEntries = useMemo(() => Object.entries(issuesData || {})
    .filter(([key]) => selectedOrg === 'all' || key.startsWith(`${selectedOrg}/`)),
    [issuesData, selectedOrg])

  const scopedPullEntries = useMemo(() => Object.entries(pullsData || {})
    .filter(([key]) => selectedOrg === 'all' || key.startsWith(`${selectedOrg}/`)),
      ...
      {(analyticsError || apiBlocked) && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <FiAlertTriangle size={18} color="var(--red)" style={{ marginTop: 2, flexShrink: 0 }} />
            <div>
              <div style={{ fontWeight: 600, marginBottom: 2 }}>Analytics data is unavailable</div>
              <div style={{ color: 'var(--text2)', fontSize: 12 }}>
                {analyticsError || `GitHub's API allowance is exhausted. Retry after ${new Date(rateLimit.reset * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}, or add a personal access token.`}
              </div>
            </div>
          </div>
          <button type="button" onClick={() => navigate('/settings')} style={{ ...C.btn('primary'), flexShrink: 0 }}>
            Open Settings
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {!hasData && (
          <button
            ...
            style={{ ...C.btn('primary'), display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, opacity: analyticsLoading || apiBlocked ? 0.65 : 1 }}
          >
            <FiRefreshCw size={13} />
      ...
      {!hasData && !govLoading && (
        <InfoBox>
          ...
          <p style={{ fontSize: 13, marginBottom: 12 }}>
            Click "Load Analytics Data" above to fetch issue and pull request history for the selected organization scope.
          </p>
          <p style={{ fontSize: 12 }}>
            Without a token, the dashboard samples the top five repositories per organization to stay within GitHub's
            anonymous API limit. With a token, it can analyze every repository.
          </p>
        </InfoBox>
      )}
      ...
      {govLoading && (
        <InfoBox>
          <div style={{ fontSize: 14, color: 'var(--text)' }}>
            Fetching issue and pull request history for {pat ? 'all repositories' : 'the top five repositories per organization'}...
          </div>
        </InfoBox>
      )}
    ...
    <div style={{ padding: '32px 24px', maxWidth: 1100, margin: '0 auto' }} className="fade-up">
      ...
      {!hasPullsData && !advanceAnalyticsLoading && (
        <InfoBox>
          ...
          <p style={{ fontSize: 13, marginBottom: 12 }}>
            Click "Load Analytics Data" above to fetch pull request data for the selected organization scope.
          </p>
          <p style={{ fontSize: 12 }}>
            Average merge time and acceptance rate are calculated from completed pull requests. Open pull requests
            are intentionally excluded from the acceptance calculation.
          </p>
        </InfoBox>
      )}
      ...
      {advanceAnalyticsLoading && (
        <InfoBox>
          <div style={{ fontSize: 14, color: 'var(--text)' }}>
            Fetching pull request history for {pat ? 'all repositories' : 'the top five repositories per organization'}...
          </div>
        </InfoBox>
      )}
        ...
        >
          <div style={{ ...C.card, minHeight: 360 }}>
            <div className="flex gap-4 items-center mb-4">
              <div style={{ width: 54, height: 54, borderRadius: 14, background: 'rgba(34,197,94,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <>
                <div style={{ position: 'relative', height: 230 }}>
                  <ResponsiveContainer width="100%" height={230}>
                    <RadialBarChart data={mergeGauge} innerRadius="72%" outerRadius="100%" startAngle={180} endAngle={0} barSize={16}>
                      <PolarAngleAxis type="number" domain={[0, 100]} tick={false} />
                      <RadialBar background={{ fill: 'var(--surface2)' }} dataKey="value" cornerRadius={12} fill={getMergeColor(advancedMetrics.avgMergeDays)} />
                    </RadialBarChart>
                  </ResponsiveContainer>
                  <div style={{ position: 'absolute', inset: '92px 0 auto', textAlign: 'center' }}>
                    <div style={{ fontSize: 34, fontWeight: 700 }}>{advancedMetrics.avgMergeDays.toFixed(1)}</div>
                    <div style={{ color: 'var(--text2)', fontSize: 13 }}>days</div>
                    <div style={{ marginTop: 5, fontSize: 12, color: getMergeColor(advancedMetrics.avgMergeDays), fontWeight: 600 }}>
                ...
                <div style={{ textAlign: 'center', color: 'var(--text2)', fontSize: 12 }}>
                  Based on <b style={{ color: 'var(--text)' }}>{advancedMetrics.merged}</b> merged pull requests
                </div>
              </>
            ) : (
              <div style={{ minHeight: 240, display: 'grid', placeItems: 'center', textAlign: 'center', color: 'var(--text2)' }}>
                <div>
                  <div style={{ color: 'var(--text)', fontWeight: 600, marginBottom: 6 }}>No merged pull requests found</div>
                  <div style={{ fontSize: 12 }}>There is not enough completed data to calculate merge time for this selection.</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ ...C.card, minHeight: 360 }}>
            <div className="flex gap-4 items-center mb-4">
              <div style={{ width: 54, height: 54, borderRadius: 14, background: 'rgba(168,85,247,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <HiCheck size={28} color="#a855f7" />
              </div>
              <div>
                <div style={{ fontWeight: 600 }}>Pull Request Acceptance Rate</div>
                <div style={{ color: 'var(--text2)', fontSize: 12 }}>Merged versus closed without merge</div>
              </div>
            </div>
            ...
            {completedPulls > 0 ? (
              <>
                <div style={{ marginTop: 12, color: 'var(--text2)', fontSize: 13 }}>
                  ...
                  <b style={{ color: 'var(--text)' }}>{advancedMetrics.rejected}</b> closed without merge
                </div>
                ...
                <div style={{ textAlign: 'center', fontSize: 28, fontWeight: 700, color: 'var(--purple)' }}>
              ...
              <div style={{ minHeight: 240, display: 'grid', placeItems: 'center', textAlign: 'center', color: 'var(--text2)' }}>
                <div>
                  <div style={{ color: 'var(--text)', fontWeight: 600, marginBottom: 6 }}>No completed pull requests found</div>
                  <div style={{ fontSize: 12 }}>Open pull requests are excluded because their outcome is not known yet.</div>
                </div>
              </div>
            )}
          </div>
