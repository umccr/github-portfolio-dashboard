...
export default function RepositoriesPage() {
  ...
  const allRepos = useMemo(() => model?.totalRepos ?? [], [model])
  const scopedRepos = useMemo(
    () =>
      selectedOrg === 'all' ? allRepos : allRepos.filter(repo => repo.orgLogin === selectedOrg),
  ...
  const filtered = useMemo(
    () =>
      scopedRepos.filter(
        r =>
          (activityClassification === 'All' ||
            r.activityClassification === activityClassification) &&
  ...
  const pinnedFirst = useMemo(
    () =>
      [...sorted].sort(
        (a, b) => Number(pinnedSet.has(String(b.id))) - Number(pinnedSet.has(String(a.id))),
      ),
    [pinnedSet, sorted],
  )
        ...
        {openInfo && (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 12 }}>
              UMCCR GitHub Portfolio evaluates repositories using issue health, contributor
              diversity, and activity classification status.
            </p>
          ...
          {ACTIVITY_CLASSIFICATIONS.map(l => (
            <button
              ...
              style={{
                ...
                background:
                  activityClassification === l
                    ? ACTIVITY_COLORS[l] || 'var(--accent)'
                    : 'transparent',
                  ...
                  >
                    <td style={{ padding: '10px 8px', textAlign: 'center' }}>
                      ...
                      >
                        <FiStar
                          size={15}
                          fill={pinnedSet.has(String(r.id)) ? 'currentColor' : 'none'}
                        />
                      </button>
                    </td>
                    ...
                    <td
                      style={{
                        padding: '10px 14px',
                        fontSize: 13,
                        color: r.open_issues_count > 30 ? 'var(--red)' : 'var(--text2)',
                      }}
                    >
                      {r.open_issues_count}
                    </td>
