...
export default function GovernancePage() {
  const { model, issuesData, runAudit, govLoading, auditComplete, loading, runGovernanceAnalysis, staleRepoStats, selectedOrg } = useApp()
  const [tab, setTab] = useState('dead')

  const ITEMS_PER_PAGE = 10
  const [stalePage, setStalePage] = useState(1)
  const scopedStaleRepoStats = useMemo(() => selectedOrg === 'all'
    ? staleRepoStats
  ...
  const scopedRepos = model.totalRepos.filter(repo => selectedOrg === 'all' || repo.orgLogin === selectedOrg)
  ...
  return (
    <div style={{ padding: '32px 24px', maxWidth: 1100, margin: '0 auto' }} className="fade-up">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 24 }}>
        ...
        <StatBox label="Stale Issues Ratio" value={`${staleIssuesRatio.toFixed(2)}%`} sub={`of ${normalIssues.length} total issues`} color={getStatus(staleIssuesRatio).color} />
      ...
      <div style={{ ...C.card, marginBottom: 20 }}>
        ...
        {topRepos.map(r => {
          const repoIssues = allIssues.filter(i => i.repoName === r.name && i.orgName === r.orgLogin)
      ...
      <div style={C.card}>
        ...
        {tab === 'dead' && (
          ...
          ) : <EmptyOk msg="No dead issues found" sub="No open issues older than 90 days in this scope." />
        )}
        ...
        {tab === 'zombie' && (
          ...
          ) : <EmptyOk msg="No zombie PRs found" sub="No open pull requests older than 90 days in this scope." />
        )}
