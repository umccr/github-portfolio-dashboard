...
import { cacheClear, fetchOrg, fetchRepos, fetchContributors, fetchIssues, fetchRateLimit, fetchPulls } from '../services/github'
import { buildAnalyticalModel, getTopRepositories, selectAnalyticsRepositories } from '../services/analytics'
...
export function AppProvider({ children }) {
  const [pat, setPat] = useState(() => sessionStorage.getItem(STORAGE_KEYS.token) || '')
  ...
  const togglePinnedRepo = useCallback(repoId => {
    ...
    setPinnedRepoIds(current => current.includes(id)
  ...
  const explore = useCallback(async () => {
    ...
    try {
      ...
      const validOrgs = await Promise.all(orgNames.map(n => fetchOrg(n, pat)))
      setOrgs(validOrgs)

      setLoadMsg('Fetching repositories...')
      const reposPerOrg = {}
      await Promise.all(validOrgs.map(async org => {
      ...
      const messages = {
        RATE_LIMIT: 'GitHub API rate limit reached. Add a PAT in Settings for 5,000 requests per hour.',
        FORBIDDEN: 'GitHub denied this request. Check the token permissions and organization approval.',
        NOT_FOUND: 'A configured organization could not be found or is not accessible with this token.',
      }
      setError(messages[err.message] || 'The dashboard could not load GitHub data. Please try again.')
      return false
    } finally {
  ...
  const fetchRepoHistory = useCallback(async (allRepos, fetcher) => {
    ...
    for (let i = 0; i < repos.length; i += 5) {
      ...
      const results = await Promise.allSettled(batch.map(async repo => ({
        ...
        data: await fetcher(repo.orgLogin, repo.name, pat),
      })))
  ...
  const runAudit = useCallback(async () => {
    ...
    try {
      ...
      setAuditComplete(!!pat && selectedOrg === 'all' && result.failedCount === 0)
    } finally {
      setGovLoading(false)
    }
  }, [model, pat, selectedOrg, govLoading, auditRepos])
  ...
  }, [model, runFullExplore, auditRepos, pat, selectedOrg, govLoading])
  ...
  const runAdvanceAnalytics = useCallback(async () => {
    ...
    try {
      ...
      setAnalyticsError(historyWarning([result]))
      setAdvanceAnalyticsComplete(!!pat && selectedOrg === 'all' && result.failedCount === 0)
    } finally {
      setAdvanceAnalyticsLoading(false)
    }
  }, [model, runFullExplore, fetchRepoHistory, pat, selectedOrg, advanceAnalyticsLoading])
  ...
  const runFullAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const complete = !!pat && selectedOrg === 'all'
      setAuditComplete(complete && issuesResult.failedCount === 0)
      setAdvanceAnalyticsComplete(complete && pullsResult.failedCount === 0)
    } finally {
      setGovLoading(false)
      setAdvanceAnalyticsLoading(false)
    }
  }, [model, runFullExplore, auditRepos, fetchRepoHistory, pat, selectedOrg, govLoading, advanceAnalyticsLoading])
  ...
  return (
    <AppContext.Provider value={{
      ...
      pinnedRepoIds, togglePinnedRepo, selectedOrg, selectOrganization,
